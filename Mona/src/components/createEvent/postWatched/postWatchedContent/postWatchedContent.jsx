...
import styles from './postWatchedContent.module.css';

function PostWatchedContent(props) {
    ...
    const minHeightStyleBlock = props.isModal ? { minHeight: props.isPublic ? '820px' : '300px' } : {};

    const articleStyle = props.isModal ? `${`${styles.modalContent} ${`dialog-ev`}`}` : "";

    return (
        <article className={articleStyle} style={minHeightStyleBlock}>
            <MovieEventHeader
...
export default PostWatchedContent;
